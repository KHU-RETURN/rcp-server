...
func (_q *AppQuery) QueryInstance() *InstanceQuery {
...
func (_q *AppQuery) WithInstance(opts ...func(*InstanceQuery)) *AppQuery {
	query := (&InstanceClient{config: _q.config}).Query()
...
func (_q *AppQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*App, error) {
	...
	if query := _q.withInstance; query != nil {
		if err := _q.loadInstance(ctx, query, nodes, nil,
			func(n *App, e *Instance) { n.Edges.Instance = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AppQuery) loadInstance(ctx context.Context, query *InstanceQuery, nodes []*App, init func(*App), assign func(*App, *Instance)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*App)
	for i := range nodes {
		if nodes[i].instance_app == nil {
			continue
		}
		fk := *nodes[i].instance_app
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "instance_app" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
