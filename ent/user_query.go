...
func (_q *UserQuery) QueryInstances() *InstanceQuery {
...
// QueryKeypairs chains the current query on the "keypairs" edge.
func (_q *UserQuery) QueryKeypairs() *KeyPairQuery {
...
func (_q *UserQuery) WithInstances(opts ...func(*InstanceQuery)) *UserQuery {
	query := (&InstanceClient{config: _q.config}).Query()
...
// WithKeypairs tells the query-builder to eager-load the nodes that are connected to
// the "keypairs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithKeypairs(opts ...func(*KeyPairQuery)) *UserQuery {
	query := (&KeyPairClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withInstances; query != nil {
		if err := _q.loadInstances(ctx, query, nodes,
			func(n *User) { n.Edges.Instances = []*Instance{} },
			func(n *User, e *Instance) { n.Edges.Instances = append(n.Edges.Instances, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withKeypairs; query != nil {
...
func (_q *UserQuery) loadInstances(ctx context.Context, query *InstanceQuery, nodes []*User, init func(*User), assign func(*User, *Instance)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.InstancesTable)
		s.Join(joinT).On(s.C(instance.FieldID), joinT.C(user.InstancesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.InstancesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.InstancesPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Instance](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
...
func (_q *UserQuery) loadKeypairs(ctx context.Context, query *KeyPairQuery, nodes []*User, init func(*User), assign func(*User, *KeyPair)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*User)
	nids := make(map[uuid.UUID]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.KeypairsTable)
		s.Join(joinT).On(s.C(keypair.FieldID), joinT.C(user.KeypairsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.KeypairsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.KeypairsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*KeyPair](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "keypairs" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
