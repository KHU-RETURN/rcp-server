...
func (_u *InstanceUpdate) check() error {
	if v, ok := _u.mutation.SyncState(); ok {
		if err := instance.SyncStateValidator(v); err != nil {
			return &ValidationError{Name: "sync_state", err: fmt.Errorf(`internal: validator failed for field "Instance.sync_state": %w`, err)}
		}
	}
	return nil
}
...
func (_u *InstanceUpdateOne) sqlSave(ctx context.Context) (_node *Instance, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`internal: missing "Instance.id" for update`)}
	}
